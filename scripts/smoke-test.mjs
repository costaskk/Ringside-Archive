...
const required=[
  'index.html','manifest.webmanifest','favicon.svg','service-worker.js','vercel.json','.env.example','SECURITY-NOTICE.md',
  'src/app.js','src/styles.css','src/storage.js','src/tvmaze.js','src/utils.js','src/records.js','src/integrations.js','src/cloud.js',
  'data/promotions.json','data/programmes.json','data/major-events.json','data/recommendations.json','data/wrestlers.json','data/format-labels.json','data/artwork-overrides.json','data/artwork-catalog.json','data/event-details.json','data/custom-records.json','data/meta.json','data/tvmaze/index.json',
  ...
  'tools/export-plex-library.ps1','scripts/audit-plex-export.mjs','scripts/plex-match-smoke.mjs','scripts/plex-storage-smoke.mjs','scripts/runtime-smoke.mjs','scripts/cloud-smoke.mjs','scripts/integration-smoke.mjs','scripts/sync-tvmaze.mjs','scripts/discover-tvmaze.mjs','scripts/scan-artwork.mjs','scripts/enrich-event-details.mjs'
];
...
if(packageJson.version!=='5.3.0')throw new Error(`Expected package version 5.3.0, found ${packageJson.version}.`);
...
for(const marker of ['ringside-archive-v5.3.0','networkFirst','url.pathname.startsWith(\'/api/\')'])if(!serviceWorker.includes(marker))throw new Error(`Service worker is missing reliability marker: ${marker}`);
for(const marker of ['activeFilters','companyLogo','wrestlerHeadshot','data-wrestler-sort','data-plex-section','refresh-integration-config','matchedPlexItems','displayArtworkUrl','Unsafe legacy Plex export detected'])if(!app.includes(marker))throw new Error(`app.js is missing v5.3 feature marker: ${marker}`);
...
for(const marker of ['.activeFilters','.companyLogo','.wrestlerHeadshot','.plexSectionList'])if(!styles.includes(marker))throw new Error(`styles.css is missing ${marker}`);
