...
const required=[
  ...
  'data/core.json','data/free-links.json','data/series-coverage-audit.json','data/promotions.json','data/programmes.json','data/major-events.json','data/recommendations.json','data/wrestlers.json','data/format-labels.json','data/artwork-overrides.json','data/artwork-catalog.json','data/event-details.json','data/custom-records.json','data/meta.json','data/tvmaze/index.json',
  ...
  'tools/export-plex-library.ps1','tools/upload-artwork-r2.ps1','scripts/cache-artwork-assets.mjs','docs/CLOUDFLARE-R2-ARTWORK.md','.github/workflows/publish-artwork-r2.yml','scripts/audit-plex-export.mjs','scripts/audit-free-links.mjs','scripts/async-ui-smoke.mjs','scripts/plex-match-smoke.mjs','scripts/plex-storage-smoke.mjs','scripts/runtime-smoke.mjs','scripts/performance-smoke.mjs','scripts/build-core.mjs','scripts/cloud-smoke.mjs','scripts/integration-smoke.mjs','scripts/sync-tvmaze.mjs','scripts/discover-tvmaze.mjs','scripts/scan-artwork.mjs','scripts/enrich-event-details.mjs'
];
...
if(packageJson.version!=='5.7.0')throw new Error(`Expected package version 5.7.0, found ${packageJson.version}.`);
...
for(const marker of ['ringside-archive-v5.7.0','networkFirst','url.pathname.startsWith(\'/api/\')'])if(!serviceWorker.includes(marker))throw new Error(`Service worker is missing reliability marker: ${marker}`);
...
for(const marker of ['.activeFilters','.companyLogo','.wrestlerHeadshot','.plexSectionList','.wrestlerProfileHero','.topMatchGrid','.deviceCodeCopy','.buttonSpinner','.asyncTaskDock','.freeWatchLink'])if(!styles.includes(marker))throw new Error(`styles.css is missing ${marker}`);
