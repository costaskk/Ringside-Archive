...
try{
  ...
  globalThis.fetch=async url=>{
    if(String(url).includes('/oauth/device/code'))return new Response(JSON.stringify({error:'forbidden'}),{status:403,headers:{'content-type':'application/json'}});
  ...
  globalThis.fetch=async url=>{
    ...
    if(value==='https://relay.plex.services/library/sections')return new Response(JSON.stringify({MediaContainer:{Directory:[{key:'7',title:'Wrestling',type:'show'}]}}),{status:200,headers:{'content-type':'application/json'}});
    if(value.startsWith('https://relay.plex.services/library/sections/7/all'))return new Response(JSON.stringify({MediaContainer:{totalSize:1,size:1,Metadata:[{title:'Episode One',grandparentTitle:'WWE Raw',type:'episode',ratingKey:'99',parentIndex:1,index:1,duration:3600000,viewOffset:1800000,thumb:'/library/metadata/99/thumb'}]}}),{status:200,headers:{'content-type':'application/json'}});
  ...
  res=responseRecorder();
  await plexLibrary({method:'POST',headers:{},body:{action:'scan',clientId:'client',token:'token',server,sectionKeys:['7']}},res);
  if(res.record.statusCode!==200||res.record.body?.items?.length!==1||res.record.body?.server?.uri!=='https://relay.plex.services')throw new Error('Plex connection fallback/section scan test failed.');

  console.log('Integration smoke passed: Trakt diagnostics, artwork logo/headshot discovery and Plex connection fallback validated.');
} finally {
