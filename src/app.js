...
function connectionsModal(){
  ...
  const diagnostics=`<div class="integrationDiagnostics"><span class="${traktReady?'ready':'missing'}"><b>Trakt</b>${traktReady?'Configured':'Missing Vercel variables'}</span><span class="${config.encryptedIntegrationStorage?'ready':'missing'}"><b>Roaming vault</b>${config.encryptedIntegrationStorage?'Configured':'Incomplete Supabase server setup'}</span><span class="${artworkReady?'ready':'optional'}"><b>TMDB artwork</b>${artworkReady?'Configured':'Optional — Wikipedia fallback active'}</span><span class="${config.r2ArtworkConfigured?'ready':'optional'}"><b>Cloudflare R2</b>${config.r2ArtworkConfigured?'Artwork CDN configured':'Optional static artwork CDN'}</span></div>`;
